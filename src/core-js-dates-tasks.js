...
function getDayName(date) {
  ...
  return namesDayArr[new Date(`${date}`).getDay()];
}
...
function getNextFriday(date) {
  ...
  const diffDay = 5 - currentDay;
  if (diffDay > 0) {
    outputDate = new Date(
      date.valueOf() + diffDay * 24 * 60 * 60 * 1000
    ).toISOString();
  } else {
    outputDate = new Date(
      date.valueOf() + (diffDay + 7) * 24 * 60 * 60 * 1000
    ).toISOString();
  }
...
function getCountDaysInMonth(month, year) {
  return new Date(
    new Date(year, month).valueOf() - 24 * 60 * 60 * 1000
  ).getDate();
}
...
function formatDate(date) {
  ...
  if (day.getUTCHours() === 12)
    ans = `${
      day.getUTCMonth() + 1
    }/${day.getUTCDate()}/${day.getUTCFullYear()}, ${day.getUTCHours()}:${day
      .getUTCMinutes()
      .toString()
      .padStart(2, '0')}:${day.getUTCSeconds().toString().padStart(2, '0')} PM`;
  else if (day.getUTCHours() > 12)
    ans = `${
      day.getUTCMonth() + 1
    }/${day.getUTCDate()}/${day.getUTCFullYear()}, ${
      day.getUTCHours() - 12
    }:${day.getUTCMinutes().toString().padStart(2, '0')}:${day
      .getUTCSeconds()
      .toString()
      .padStart(2, '0')} PM`;
  else
    ans = `${
      ...
      .getUTCMinutes()
      .toString()
      .padStart(2, '0')}:${day.getUTCSeconds().toString().padStart(2, '0')} AM`;
  return ans;
}
...
function getWeekNumberByDate(date) {
  let diffDay;
  if (new Date(date.getFullYear(), 0, 1).getDay() > 0)
    diffDay = new Date(date.getFullYear(), 0, 1).getDay() - 1;
  else diffDay = 6.01;
  return Math.ceil(
    (date.getTime() -
      new Date(date.getFullYear(), 0).getTime() +
...
function getNextFridayThe13th(date) {
  ...
  let daysToFriday;
  if (date.getDay() < 5) daysToFriday = 5 - date.getDay();
  else daysToFriday = 7 - (date.getDay() - 5);
  let firstFri = new Date(date.getTime() + dayInMiliSec * daysToFriday);
  while (firstFri.getDate() !== 13 || firstFri.getDay() !== 5) {
    firstFri = new Date(firstFri.getTime() + 7 * dayInMiliSec);
  }
  return firstFri;
}
...
function getQuarter(date) {
  let ans;
  if (date.getMonth() < 3) ans = 1;
  else if (date.getMonth() < 6) ans = 2;
  else if (date.getMonth() < 9) ans = 3;
  else ans = 4;
  return ans;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const endDate = new Date(period.end.split('-').reverse().join(',')).getTime();
  while (currentDate <= endDate) {
    ...
    currentDate += oneDay * countOffDays;
  }
  return SheduleArr;
}
